...
# Slightly simplified options for the hog function in skimage.feature.
...
# Reduced resolution version of the image as a feature.
...
# Three histograms, one for each layer of an image. Then convatenate into one feature
...
# Input: image as np.array and list of coord-pairs. Output: image with drawn rectangles
...
# This function converts one image into features
...
def search_windows(img, boxes, classifier, dec_threshold=0.75):
    ...
    for box in boxes:
        ...
        test_img = cv2.resize(img[box[0][1]:box[1][1], box[0][0]:box[1][0]], (64, 64))
        ...
        dec = classifier.predict(test_features)
...
def update_heatmap(candidates, heatmap):
